...
export const createCarDetails: RequestHandler = async (req, res) => {
    ...
    const car = await CarModel.create({make, year, VIN, model, owner: req.user.id})
...
export const getCarDetails: RequestHandler = async (req, res) => {
    ...
    if(!cars) return sendErrorRes(res, "unauthorize request, no car found!", 401)
...
export const updateCarDetails: RequestHandler = async (req, res) =>{
    ...
    if(!user) return sendErrorRes(res, "unauthorize requst, no car record found!", 403)
    const car = await CarModel.findByIdAndUpdate(carId, {make, model, year, VIN})
    if(!car) return sendErrorRes(res, "unauthorize requst, no car record found!", 403)
